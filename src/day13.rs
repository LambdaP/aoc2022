...
impl Aoc for Day13 {
    fn part1(&self, lines: &[&[u8]]) -> Result<Box<dyn Display>> {
        ...
        let res: usize = pairs.into_iter().enumerate().map(|(i,(x,y))| (i+1) * usize::from(x <= y)).sum();
        result!(res)
    }

    fn part2(&self, lines: &[&[u8]]) -> Result<Box<dyn Display>> {
        let pairs = parse(lines)?;
        let mut lists = pairs.into_iter().flat_map(|(x,y)| [x,y]).collect::<Vec<_>>();
...
fn parse_line(line: &[u8]) -> Result<List<u8>> {
    ...
    stack.pop().map(List::Vec).ok_or_else(|| eyre!("parse error"))
}
